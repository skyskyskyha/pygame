...
pygame.display.set_caption('Hapi Bird')
...
def generate_chunk(x,y,randint):
    chunk_data = []
    for y_pos in range(CHUNK_SIZE):
        for x_pos in range(CHUNK_SIZE):
            ...
            tile_type = 0 # nothing
            if (target_x % 3 ==0 and not (target_y >= randint and target_y <= randint + 3)):
                tile_type = 2
            if (tile_type):
...
while True: # game loop
    ...
    tile_rects = []
    for x in range(8):
        randint = random.randint(2,8)
        for y in range(6):
            target_x = x - 1 + int(round(scroll[0]/(CHUNK_SIZE*16)))
            target_y = y - 1 + int(round(scroll[1]/(CHUNK_SIZE*16)))
            target_chunk = str(target_x) + ';' + str(target_y)
            if target_chunk not in game_map:
                game_map[target_chunk] = generate_chunk(target_x,target_y,randint)
